...
public class EnemyRoaming : BaseState
{
	...
	public override void UpdateLogic()
	{
		...
		//Костыль: Тип врага сейчас определяется if else. При надобности переписать! 
		if (_sm.CheckPlayerContact(48, 1, 30))
		{
			// переключаемся в другое состояние в зависимости от того, является ли агент NPC
			if (!_sm.isAnNPC && !_sm.debugMode && !_sm.isOneShot && !_sm.isMinigunner && _sm.playerObject != null)
				stateMachine.ChangeState(_sm.chasingState);
			else if (_sm.isOneShot)
				_sm.ChangeState(_sm.oneShotChasingState);
		}

		//если в состоянии страха то в зависимости от поведения сменить состояние
		if (_sm.isAlerted && !_sm.isAnNPC && !_sm.isOneShot && !_sm.isMinigunner && _sm.playerObject != null)
		...
		else if (_sm.isAlerted && _sm.isOneShot)
			_sm.ChangeState(_sm.oneShotChasingState);
	}
